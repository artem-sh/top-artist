...
import sh.app.topartist.core.util.ResourceUtil.resourceNextToMe
...
class PromodjRuRatingParserTest extends FlatSpec with ShouldMatchers {
  ...
  it should ("throw RatingParserException if promorank occures once in web page but no value defined") in {
    evaluating {
      parseRating("""
      <html>
        <body>
  ...
  it should ("throw RatingParserException if promoranks occures twice in web page") in {
    evaluating {
      parseRating("""
      <html>
        <body>
